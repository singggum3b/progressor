...
var Progressor = (function () {
	function Progressor(options) {
		...
		this.output = _terminalKit2.default.terminal;
   ...
   */
		...
		value: function display() {
			...
			this.update(this.format.replace(/%([a-z\-_]+)(?:\:([^%]+))?%/g, function (matches, part1, part2) {
				var text = undefined;
				var formatter = _this2.getPlaceholderFormatterDefinition(part1);
				if (formatter) {
					text = formatter(_this2);
				} else if (_this2.messages.hasOwnProperty(part1)) {
					text = _this2.messages[part1];
				} else {
					return matches;
				}
   ...
   */
		...
		value: function update(message) {
			...
			if (null !== this.lastMessagesLength) {
				for (var index in lines) {
					...
					if (this.lastMessagesLength > _helpers.Helpers.strlenWithoutDecoration(line)) {
						lines[index] = (0, _pad2.default)(line, this.lastMessagesLength, "\x20");
					}
				}
			}
			...
			if (this.formatLineCount) {
				...
				this.output((0, _sprintfJs.sprintf)("\x1b[%dA", this.formatLineCount));
			}
			...
			for (var index in lines) {
				var line = lines[index];
				var len = _helpers.Helpers.strlenWithoutDecoration(line);
			...
			var placeholders = {
				'bar': function bar(_bar) {
					...
					if (completeBars < _bar.options.barWidth) {
						var emptyBars = _bar.options.barWidth - completeBars - _helpers.Helpers.strlenWithoutDecoration(_bar.options.progressChar);
				...
				'elapsed': function elapsed(bar) {
					...
					return _helpers.Helpers.formatTime(seconds);
				},
				...
				'estimated': function estimated(bar) {
					...
					return _helpers.Helpers.formatTime(estimated);
				},
				...
				'memory': function memory(bar) {
					...
					return _helpers.Helpers.formatMemory(memoryUsage.rss);
				},
				'remaining': function remaining(bar) {
					...
					return _helpers.Helpers.formatTime(remaining);
				}
			};
		...
		value: function start() {
			...
			if (this.options.beforeNewlines) {
				this.output((0, _repeatString2.default)("\n", parseInt(this.options.beforeNewlines)));
			}
